...
## Coding & UI Rules
- **Languages**: Use English for all code comments, but response in native operating system language.
...
## Performance & Context
- **Code Integrity**: Prioritize a complete, working solution over conciseness. Rewrite entire methods if the current implementation is fundamentally broken.
...
- **Build APK Procedure**: Only build the APK after code changes. Execute: `.\gradlew assembleDebug; Get-Date -Format "HH:mm:ss (yyyy-MM-dd)"`
...
- **Debugging**: Focus on Logic and NullPointerExceptions first. Use command-line tools for file management within the project.
...
- Do not install/run the app on the emulator unless explicitly asked (especially if the emulator is off - it kills my resources).
