...
public class KeyboardSwitcher implements
    ...
    public static final int KEYBOARDMODE_WEB_NOTAB = R.id.mode_webentry_notab;
    public static final int KEYBOARDMODE_WEB_NOTAB_WITH_SETTINGS_KEY = R.id.mode_webentry_notab_with_settings_key;

    // Symbols keyboard layout without the settings key
    public static final int KEYBOARDMODE_SYMBOLS = R.id.mode_symbols;
    public static final int KEYBOARDMODE_SYMBOLS_WEB = R.id.mode_symbols_web;
    ...
    public static final int KEYBOARDMODE_SYMBOLS_WEB_WITH_SETTINGS_KEY = R.id.mode_symbols_web_with_settings_key;
    public static final int KEYBOARDMODE_SYMBOLS_WEB_NOTAB = R.id.mode_symbols_web_notab;
    public static final int KEYBOARDMODE_SYMBOLS_WEB_NOTAB_WITH_SETTINGS_KEY = R.id.mode_symbols_web_notab_with_settings_key;
    ...
    private static final int[] ALPHABET_MODES = { KEYBOARDMODE_NORMAL,
            ...
            KEYBOARDMODE_WEB_NOTAB,
            KEYBOARDMODE_WEB_NOTAB_WITH_SETTINGS_KEY,
            KEYBOARDMODE_SYMBOLS_WEB_NOTAB,
            KEYBOARDMODE_SYMBOLS_WEB_NOTAB_WITH_SETTINGS_KEY };
    ...
    public static void init(LatinIME ims) {
        ...
        sInstance.mReverseTab = prefs.getBoolean("pref_reverse_tab_key", false);
...
//            }
    ...
    private KeyboardId getKeyboardId(int mode, int imeOptions, boolean isSymbols) {
        ...
        mReverseTab = LatinIME.sKeyboardSettings.reverseTab;
        if (mFullMode > 0) {
            switch (mode) {
            case MODE_TEXT:
            case MODE_URL:
        ...
        if (isSymbols) {
            if (mode == MODE_PHONE) {
                ...
                int symMode;
                if (mReverseTab && mode == MODE_WEB) {
                    symMode = mHasSettingsKey ? KEYBOARDMODE_SYMBOLS_WEB_WITH_SETTINGS_KEY
                            : KEYBOARDMODE_SYMBOLS_WEB;
                } else {
                    symMode = mHasSettingsKey ? KEYBOARDMODE_SYMBOLS_WITH_SETTINGS_KEY
                            : KEYBOARDMODE_SYMBOLS;
                }
                return new KeyboardId(KBD_SYMBOLS, symMode, false, hasVoice);
            }
        }
        switch (mode) {
        case MODE_NONE:
        ...
        case MODE_SYMBOLS:
            int symbolsMode;
            if (mReverseTab) {
                symbolsMode = mHasSettingsKey ? KEYBOARDMODE_SYMBOLS_WEB_NOTAB_WITH_SETTINGS_KEY
                        : KEYBOARDMODE_SYMBOLS_WEB_NOTAB;
            } else {
                symbolsMode = mHasSettingsKey ? KEYBOARDMODE_SYMBOLS_WITH_SETTINGS_KEY
                        : KEYBOARDMODE_SYMBOLS;
            }
            return new KeyboardId(KBD_SYMBOLS, symbolsMode, false, hasVoice);
        case MODE_PHONE:
        ...
        case MODE_WEB:
            int webMode;
            if (mReverseTab) {
                webMode = mHasSettingsKey ? KEYBOARDMODE_WEB_NOTAB_WITH_SETTINGS_KEY
                        : KEYBOARDMODE_WEB_NOTAB;
            } else {
                webMode = mHasSettingsKey ? KEYBOARDMODE_WEB_WITH_SETTINGS_KEY
                        : KEYBOARDMODE_WEB;
            }
            return new KeyboardId(keyboardRowsResId, webMode, true, hasVoice);
        }
        return null;
    }
    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences,
        ...
        if (PREF_KEYBOARD_LAYOUT.equals(key)) {
            ...
            recreateInputView();
        } else if ("pref_reverse_tab_key".equals(key)) {
            mReverseTab = sharedPreferences.getBoolean(key, false);
            recreateInputView();
        }
    }
