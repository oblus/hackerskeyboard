    ...
    private final void loadDictionary(InputStream[] is) {
        try {
            ...
            for (int i = 0; i < is.length; i++) {
                // available() is unreliable for compressed streams
                total += 1024 * 1024; // assume 1MB initial guess
            }
            ...
            for (int i = 0; i < is.length; i++) {
                ...
                int read;
                while ((read = is[i].read(buffer)) != -1) {
                    if (got + read > mNativeDictDirectBuffer.capacity()) {
                        // Reallocate if we underestimated
                        int newCapacity = Math.max(mNativeDictDirectBuffer.capacity() * 2, got + read);
                        ByteBuffer newBuffer = ByteBuffer.allocateDirect(newCapacity).order(ByteOrder.nativeOrder());
                        mNativeDictDirectBuffer.flip();
                        newBuffer.put(mNativeDictDirectBuffer);
                        mNativeDictDirectBuffer = newBuffer;
                    }
                    mNativeDictDirectBuffer.put(buffer, 0, read);
                    got += read;
                }
            }
            ...
            if (got > 0) {
                mNativeDict = openNative(mNativeDictDirectBuffer,
                        TYPED_LETTER_MULTIPLIER, FULL_WORD_FREQ_MULTIPLIER, got);
                mDictLength = got;
            }
            if (mDictLength > 100) Log.i("PCKeyboard", "Loaded dictionary, len=" + mDictLength);
        } catch (Exception e) {
            Log.e("PCKeyboard", "Error loading binary dictionary", e);
        } finally {
            try {
                if (is != null) {
                    for (int i = 0; i < is.length; i++) {
